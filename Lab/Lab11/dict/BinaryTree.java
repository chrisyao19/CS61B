...
public class BinaryTree implements Dictionary {
  ...
  private BinaryTreeNode findHelper(Comparable key, BinaryTreeNode node) {
    ...
    if(node!=null){
      int tmp = key.compareTo(node.entry.key());
      if(tmp==0){
        return node;
      } else if(tmp<0){
  ...
  public Entry remove(Object key) {
    BinaryTreeNode tmp = findHelper((Comparable)key, root);
    System.out.println("" + (tmp==root));
    if(tmp==root){
      if(tmp.leftChild!=null && tmp.rightChild==null){
        this.root = tmp.leftChild;
      } else if(tmp.leftChild==null && tmp.rightChild!=null){
        this.root = tmp.rightChild;
      } else if(tmp.leftChild!=null && tmp.rightChild!=null){
        BinaryTreeNode s = tmp.rightChild;
        ...
        if(tmp.leftChild!=null && tmp.rightChild==null){
          ...
          //s.leftChild = tmp.leftChild; s.rightChild = tmp.rightChild;
          tmp.entry = s.entry;
          //s.parent = tmp.parent;
          //System.out.println("fuck"+s);
          // if(tmp.parent.leftChild == tmp){
          //   tmp.parent.leftChild = s;
          // } else{
          //   tmp.parent.rightChild = s;
          // }
          //System.out.println("fuck"+s);
        }
        else{
