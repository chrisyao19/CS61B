...
public class Maze {
  ...
  public Maze(int horizontalSize, int verticalSize) {
    ...
    int count = horiz*(vert-1) + (horiz-1)*vert;
    DisjointSets cell = new DisjointSets(horiz*vert);
    //initiate the wall 
    int[] wall = new int[count];
    for(int t=0;t<wall.length;t++) {
      wall[t] = t;
    }
    //create a random permutation.
    random(wall, count);
    //access to the random permutation, and break the wall if condition is satisfied.
    for(int t=0; t<count;t++){
      int x = wall[t];
      int tmp[] = cell_to_wall(wall[t]);
      boolean flag = cell.find(tmp[0]) == cell.find(tmp[1]);
      //if two cells are blocked
      if(flag == false) {
        int root1 = cell.find(tmp[0]);
        int root2 = cell.find(tmp[1]);
        cell.union(root1, root2);
        if(x<horiz*(vert-1)) {
          hWalls[x%horiz][x/horiz] = false;
        } else{
          x = x - horiz*(vert-1);
          vWalls[x/vert][x%vert] = false;
        }
      }
    }
  }
   //to produce a random permutation.
    public void random(int[] s, int count){
      while(count!=1){
        int radm = randInt(count-1);
        int tmp = s[radm];
        s[radm] = s[count-1];
        s[count-1] = tmp;
        count--;
    ...
    //to determine the 2 cells which a wall is between them.
    public int[] cell_to_wall(int s){
      int[] result = new int[2];
      if(s<horiz*(vert-1)) {
        result[0] = (s/horiz)*horiz + s%horiz;
        result[1] = result[0] + horiz;
      } else{
        s = s - horiz*(vert-1);
        result[0] = horiz*(s%vert)+(s/vert); 
        result[1] = result[0] + 1;
      }
      return result;
    }
